...
class BiometricService:
    ...
    def detect_faces_opencv(self, frame: np.ndarray) -> FaceDetectionResult:
    ...
    def extract_simple_features(self, frame: np.ndarray, face_location: List[int]) -> Optional[np.ndarray]:
    ...
    async def enroll_biometric(self, user_id: int, video_data: str, video_format: str) -> BiometricResult:
        ...
        try:
            ...
            # Process frames to find best face features
            for frame in frames:
                face_result = self.detect_faces_opencv(frame)
                ...
                if face_result.faces_detected > 0:
                    ...
                    # Extract features
                    features = self.extract_simple_features(frame, face_location)
            ...
            if best_features is None:
                return BiometricResult(
                    success=False,
                    message="No valid face features could be extracted",
                    face_detected=face_detected,
                    quality_score=best_quality
                )
            
            # Encrypt and store template
            template_data = encrypt_data(best_features.tobytes())
            template_hash = hash_data(best_features.tobytes())
